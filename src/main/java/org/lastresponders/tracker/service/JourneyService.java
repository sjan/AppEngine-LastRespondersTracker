...
public class JourneyService {
	private static final Logger log = Logger.getLogger(JourneyService.class
			.getName());
	...
	public List<TripPosition> plannedRoute() {
		...
		try {
			...
			ValueRange response = SheetsUtil.getSheet().spreadsheets().values()
			...
			if (values == null || values.size() == 0) {
				...
				while (iterator.hasNext()) {
					...
					if(row.size() >= 18) {
						...
						ret.add(new TripPosition( Double.parseDouble((String) row.get(18)), Double.parseDouble((String) row.get(19)),formattedDate));
					}
				}
			}	
		} catch (IOException e) {
